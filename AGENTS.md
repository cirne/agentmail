...
**CLI help and onboarding (no env required):** `zmail --help`, `zmail -h`, `zmail help` show usage, including when to use `zmail ask` versus search/read/thread/who/attachment (canonical string: `CLI_USAGE` in `src/lib/onboarding.ts`). Use **`zmail <command> --help`** for per-command flags. **Progressive disclosure:** JSON output from commands such as **`search`** may include a **`hint`** field (and truncation metadata); text mode may print similar tips after results—read them before inventing a new approach. For the full comparison and hybrid patterns, see [docs/ASK.md](docs/ASK.md). End-user skill copy: [skills/zmail/references/CANONICAL-DOCS.md](skills/zmail/references/CANONICAL-DOCS.md). If any command fails due to missing config, the CLI prints "No config found. Run 'zmail setup' or 'zmail wizard' first."
