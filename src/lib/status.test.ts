...
describe("formatTimeAgo", () => {
  ...
  it("formats minutes ago", () => {
    ...
    expect(result).toEqual({ human: "5 minutes ago", duration: "PT5M" });
  });

  it("formats hours ago", () => {
    const result = formatTimeAgo("2026-03-07T10:00:00Z");
    expect(result).toEqual({ human: "2 hours ago", duration: "PT2H" });
  });

  it("formats days ago", () => {
    const result = formatTimeAgo("2026-03-05T12:00:00Z");
    expect(result).toEqual({ human: "2 days ago", duration: "P2D" });
  });

  it("formats weeks ago", () => {
    const result = formatTimeAgo("2026-02-28T12:00:00Z");
    expect(result).toEqual({ human: "1 week ago", duration: "P1W" });
  });

  it("formats months ago", () => {
    const result = formatTimeAgo("2025-12-07T12:00:00Z"); // ~3 months ago
    expect(result).toEqual({ human: "3 months ago", duration: "P90D" });
  });

  it("formats years ago", () => {
    const result = formatTimeAgo("2024-03-07T12:00:00Z");
    expect(result).toEqual({ human: "2 years ago", duration: "P2Y" });
  });
