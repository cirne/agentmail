...
1. **Optional:** `zmail refresh` — pulls new mail into the **same** local index `search` uses. If the user is sure mail arrived but nothing new showed up, try `zmail refresh --force` (see `zmail refresh --help`).
2. **Always:** `zmail search …` — the code may **already** be indexed; **do not** assume it only appears in `refresh`’s new-mail preview.
...
- **Recency:** `--after` / `--before` accept **ISO dates** (`YYYY-MM-DD`) or **relative** specs like `1d`, `7d`, `2w` (same family as `zmail sync --since`; see `zmail search --help`). Example: `zmail search 'verification' --after 1d --limit 15`.
