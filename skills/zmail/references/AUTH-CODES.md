...
1. **Optional:** `zmail update` — pulls new mail into the **same** local index `search` uses.
2. **Always:** `zmail search …` — the code may **already** be indexed; **do not** assume it only appears in `update` output.
3. **Then:** `zmail read <message_id>` on the best match(es) if the snippet is not enough (codes are often in the body).

**Avoid for this task:** `zmail ask`, `zmail check`, and `zmail review` — they send mail-derived text to **OpenAI**; verification lookup is usually a **local** `search` + `read`.

---

## Search tips

- **Recency:** `--after` / `--before` accept **ISO dates** (`YYYY-MM-DD`) or **relative** specs like `1d`, `7d`, `2w` (same family as `zmail update --since`; see `zmail search --help`). Example: `zmail search 'verification' --after 1d --limit 15`.
...
After an optional `update` via CLI (or your host’s sync), use **`search_mail`** with `query`, `afterDate` / `beforeDate`, optional `fromAddress`, then **`get_message`** or batched **`get_messages`**. Inline operators in `query` include `after:`, `before:`, `from:` (see **`docs/MCP.md`** in the package/repo).
