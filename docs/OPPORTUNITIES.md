...
| [OPP-025](opportunities/OPP-025-cross-platform-agent-skills-packaging.md) | Agent Skills spec — portable `SKILL.md` + distribution | **Publishable user skill:** `skills/zmail/` → **`/zmail`**. **Repo dev skills:** `.cursor/skills/*` (internal; not shipped to end users). **Spec:** [agentskills.io/specification.md](https://agentskills.io/specification.md), `skills-ref validate`. **Hosts:** Claude Code, Cursor, OpenClaw ([AgentSkills-compatible](https://docs.openclaw.ai/tools/skills)). **Strategy:** skill + CLI default; MCP optional. |
...
| [OPP-024](opportunities/archive/OPP-024-sqlite-node-abi-mitigation.md) | SQLite / Node ABI — Global Install Reliability | **Superseded:** zmail now uses Node built-in `node:sqlite` (ADR-023). Archived doc describes the former `better-sqlite3` + `postinstall` mitigation. |
