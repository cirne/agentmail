...
| [OPP-022](opportunities/OPP-022-ask-synthesis-detail-level.md) | Ask Synthesis — Default Detail Level Too Shallow for Broad Queries | Mini's "be concise" prompt causes synthesis queries to omit specific details (dates, locations, changes). One `ask` call should return what currently takes 3 follow-up calls. Tune the mini system prompt to be query-aware: concise for lookups, thorough for broad synthesis. |
| [OPP-023](opportunities/OPP-023-ask-only-interface.md) | Ask-Only Interface — Remove Search/Read/Thread/Who Primitives | Remove 6 CLI commands and 7 MCP tools in favor of `ask` / `ask_email` as the sole query interface. Cuts surface area ~50%, eliminates BUG-018/021-class bugs, forces `ask` quality investment. Phased: add `ask_email` MCP tool → deprecate → remove. Prerequisite: OPP-022. |
