...
| [OPP-025](opportunities/OPP-025-cross-platform-agent-skills-packaging.md) | Cross-Platform Agent Skills — Cursor, Claude Code, OpenClaw | Research + **strategy:** prefer **skill + CLI** over MCP as default packaging/onboarding; MCP stays supported (IDE/OpenClaw bridge). Portable `SKILL.md` + npm carrier; phased doc migration; OpenClaw target = native/shell-backed skill, not MCP-only. |
