...
| [OPP-011](opportunities/OPP-011-send-email.md) | Send Email — Draft + SMTP | **Shipped:** SMTP + drafts + LLM **`draft edit`** + **`draft rewrite`** + MCP send/draft tools; MD→plain on send; forward inline from maildir; SMTP verify in setup/wizard. **Remaining:** MCP parity for LLM edit, OAuth/IMAP-Drafts, voice/tagline, optional CI SMTP test. Product gating: see opp doc. |
