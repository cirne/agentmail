...
**Problem:** zmail is implemented twice (Rust at repo root, Node under `node/`). End users and skills still document `npm install -g @cirne/zmail`. We need a clear path to a single supported binary (or an explicit dual-publish story) without stranding existing installs.
