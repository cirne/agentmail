...
**Example:** A user in Claude Code says "set up zmail for my Gmail." The agent should be able to (1) install the CLI (e.g. `npm i -g @cirne/zmail`), (2) guide creation of config via `zmail setup` or `.env` in `~/.zmail`, (3) run first sync and confirm success — without cloning the repo or asking the user to hunt for docs.
...
- **Local global install** — `npm run install-cli` runs `npm run build` then `npm install -g .` so `zmail` matches the published package layout (`dist/index.js`). See [AGENTS.md](../../AGENTS.md).

---

## Goals (remaining optional)

- **Stable release location** — Canonical install is `npm i -g @cirne/zmail` (see OPP-007). Per-platform binaries or tarballs are optional.
...
- **From npm:** `npm i -g @cirne/zmail` (Node.js 22.16+). Canonical install; no binary download required.
